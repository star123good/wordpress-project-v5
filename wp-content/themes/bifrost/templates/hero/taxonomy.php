...
if (get_field('hero_height', $queried_object) == '2' && get_field('hero_custom_height', $queried_object)) {
    $bifrost_hero_custom_height = get_field('hero_custom_height', $queried_object);
} else {
 ...
 * Image
 */
$bifrost_hero_image = bifrost_inherit_option('hero_image', 'hero_image', '2');

// Increase the number for 1, because the taxonomies doesn't have the option use the post/page featured image
if (get_field('hero_image', $queried_object) > 2 && class_exists('WooCommerce') && !is_shop()) {
    $bifrost_hero_image++;
}

if (class_exists('WooCommerce') && is_shop()) {
    $bifrost_hero_custom_image = get_field('hero_image', $queried_object) == '4' ? get_field('hero_custom_image', $queried_object) : get_theme_mod('hero_custom_image');
} else {
    $bifrost_hero_custom_image = get_field('hero_image', $queried_object) == '3' ? get_field('hero_custom_image', $queried_object) : get_theme_mod('hero_custom_image');
}

if ($bifrost_hero_image == '1' || $bifrost_hero_image == '2') {
...
switch (bifrost_inherit_option('hero_image_repeat', 'hero_image_repeat', '1')) {
    case '1':
        ...
        break;
    case '2':
        $bifrost_hero_style[] = 'background-repeat: repeat-all';
        break;
    case '3':
...
$bifrost_hero_image_overlay_opacity = (get_field('hero_image_overlay', $queried_object) == '2') ? get_field('hero_image_overlay_opacity', $queried_object) : get_theme_mod('hero_image_overlay_opacity');
$bifrost_hero_image_overlay_color = (get_field('hero_image_overlay', $queried_object) == '2') ? get_field('hero_image_overlay_color', $queried_object) : get_theme_mod('hero_image_overlay_color');
...
$bifrost_hero_custom_title = (get_field('hero_title', $queried_object) == '3') ? get_field('hero_custom_title', $queried_object) : get_theme_mod('hero_custom_title');
$bifrost_hero_title_markup = '';
$bifrost_hero_title_class = 'o-hero__content__title';

if (class_exists('WooCommerce') && is_shop()) {
    $bifrost_hero_title_markup = woocommerce_page_title(false);
} elseif (is_date()) {
    $bifrost_hero_title_markup = get_the_date();
} elseif (is_author()) {
    $bifrost_hero_title_markup = get_the_author();
} elseif ($bifrost_hero_title == '1') {
    $bifrost_hero_title_markup = single_term_title('', false);
 ...
 * Title Color
 */
$bifrost_hero_title_color = get_field('hero_title_color', $queried_object) == '1' ? get_theme_mod('hero_title_color', '#232931') : get_field('hero_title_color_custom', $queried_object);
